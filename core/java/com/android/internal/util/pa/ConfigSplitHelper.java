...
*               2015 CyanideL (Some small changes)
*
...
public class ConfigSplitHelper {
    ...
    public static ArrayList<ActionConfig> getActionConfigValues(Context context, String config,
        ...
        for (String configValue : config.split("\\" + PieConstants.DELIMITER_BUTTON)) {
            counter++;
            if (counter == 1) {
    ...
    public static String setActionConfig(
        ...
        for (int i = 0; i < actionConfigs.size(); i++) {
            if (i != 0) {
                finalConfig += PieConstants.DELIMITER_BUTTON;
            }
            actionConfig = actionConfigs.get(i);
            finalConfig += actionConfig.getClickAction() + PieConstants.DELIMITER_BUTTON;
            if (!isShortcut) {
                finalConfig += actionConfig.getLongpressAction()
                    + PieConstants.DELIMITER_BUTTON;
            }
